...
use strict 'vars';
use vars qw($opt_F $data $line @fields @index @z @zstagger @zDet @r @thetaDet @overlap @dx @dy @dz @tilt @roll @rollDet @rx @ry @rz @quartzCutAngle @refTopOpeningAngle @dzRef @dxLg @dyLg @dzLg  @lgTiltAngle @dxPmt @dyPmt @dzPmt @ddPmt @dtWall @extraPMTholderWidth @extraPMTholderDepth @dtReflector @numDet $i $j $k );
##-------------------------------------------------------------------------------------------------------------------##
...
$extraPMTholderWidth[$i]= trim($fields[15]);                      # Extra width of PMT holder (in addition to PMT diameter)
$extraPMTholderDepth[$i]= trim($fields[16]);                      # Extra depth of PMT holder (in addition to PMT diameter)
...
print def "${index[$j]*1000+$k+1}, $zDet[$k], ${r[$j]*sin($thetaDet[$k])}, ${r[$j]*cos($thetaDet[$k])}, $dx[$j], $dy[$j], $dz[$j], ${thetaDet[$k]*-1}, ${tilt[$j]*pi/180}, ${rollDet[$k]*pi/180},  0.785398, ${refTopOpeningAngle[$j]*pi/180}, $dzRef[$j], $dxLg[$j], $dy[$j], $dzLg[$j], ${lgTiltAngle[$j]*pi/180}, ${ddPmt[$j]*25.4*1.005}, ${ddPmt[$j]*25.4*1.005}, ${dzPmt[$j]*25.4}, ${ddPmt[$j]*25.4/2}, $dtWall[$j], ${dtWall[$j]/5}  \n";
