...
# Version 2 = PMT housing inside light guide.

##------------------Declare variables explicitly so "my" not needed.----------------##
use strict 'vars';
use vars
  qw($opt_M $opt_D $opt_T $data $line @fields $dxM $dyM $dzM $drMinM $dxMext @index @x @y @z @dx @dy @dz @rx @ry @rz @quartzCutAngle @refTopOpeningAngle @dzRef @dxLg @dyLg @dzLg  @lgTiltAngle @dxPmt @dyPmt @dzPmt @drPmt @dtWall @dtReflector $i $j $k $angle1 $angle2);
##----------------------------------------------------------------------------------##
...
"<box name=\"quartzRecSol_$index[$j]\" x=\"$dx[$j]\" y=\"$dy[$j]\" z=\"$dz[$j]\" lunit= \"mm\" />\n";
 ...
 <section zOrder=\"2\" zPosition=\"", $dy[$j] * (0.5),
    ...
    print def "<trd name = \"lgLogicSol_$index[$j]\" z=\"$dzLg[$j]\" y1=\"",
      $dyLg[$j] + 2 * $dtWall[$j] /  ( cos($angle2) ), "\" x1=\"",
      $dxLg[$j] + 2 * $dtWall[$j] /  ( cos($angle1) ), "\" y2=\"",
      $dyPmt[$j] + 2 * $dtWall[$j] / ( cos($angle2) ), "\" x2=\"",
      $dxPmt[$j] + 2 * $dtWall[$j] / ( cos($angle1) ), "\" lunit= \"mm\"/>\n";
 ...
 <section zOrder=\"2\" zPosition=\"", $dyLg[$j] * (0.5) + $dtWall[$j],
   ...
   <position name=\"lgLogicSolPos_$index[$j]\" unit=\"mm\" x=\"",
      ...
      $dzLg[$j] * 0.5 * sin( $lgTiltAngle[$j] ), "\" y=\"0\" z=\"",
      $dz[$j] * (0.5) +
      $dzRef[$j] -
      0.5 * $dxLg[$j] * sin( $lgTiltAngle[$j] ) +
      $dzLg[$j] * 0.5 * cos( $lgTiltAngle[$j] ), "\"/>
    <rotation name=\"lgLogicSolRot_$index[$j]\" unit=\"rad\" x=\"0\" y=\"",
      $lgTiltAngle[$j] * (-1), "\" z=\"0\"/> 
</union>\n";

    print def
"<trd name = \"lgSol_$index[$j]\" z=\"$dzLg[$j]\" y1=\"$dyLg[$j]\" x1=\"$dxLg[$j]\" y2=\"$dyPmt[$j]\" x2=\"$dxPmt[$j]\" lunit= \"mm\"/>\n";
 ...
 <twoDimVertex x=\"", $dx[$j] * (0.5), "\" y=\"", $dzRef[$j] * (-0.5), "\" />
 <twoDimVertex x=\"",
      $dx[$j] * (0.5) + $dtReflector[$j] / cos( $refTopOpeningAngle[$j] ),
      "\" y=\"", $dzRef[$j] * (-0.5), "\" />
 <section zOrder=\"1\" zPosition=\"", $dyLg[$j] * (-0.5),
      "\" xOffset=\"0\" yOffset=\"0\" scalingFactor=\"1\" />
 <section zOrder=\"2\" zPosition=\"", $dyLg[$j] * (0.5), "\" xOffset=\"", 0,
    ...
    print def
"<cone name = \"pmtSol_$index[$j]\" rmin1=\"0\" rmax1=\"$drPmt[$j]\" rmin2=\"0\" rmax2=\"$drPmt[$j]\" z=\"$dzPmt[$j]\"
startphi=\"0\" deltaphi=\"2*PI\" aunit=\"rad\" lunit= \"mm\" />\n";
	...
	<position unit=\"mm\" name=\"pmtLogicPos_$index[$j]\" x=\"0\" y=\"0\" z=\"",
      $dzLg[$j] * 0.5 - $dzPmt[$j] * 0.5, "\"\/>
        <rotation unit=\"rad\" name=\"pmtLogicRot_$index[$j]\" x=\"0\" y=\"0\" z=\"0\"/>
</subtraction>\n\n";
